...
int LuaScriptInterface::luaPlayerSetGuild(lua_State* L)
{
	...
	player->setGuild(*getUserdata<Guild_ptr>(L, 2));
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaGuildCreate(lua_State* L)
{
	...
	std::shared_ptr<Guild> guild = g_game.getGuild(id);
	if (guild) {
		pushUserdata<Guild_ptr>(L, &guild);
		setMetatable(L, -1, "Guild");
	} else {
		lua_pushnil(L);
	}
...
{
	// guild:getId()
	std::shared_ptr<Guild> guild = *getUserdata<Guild_ptr>(L, 1);
	if (guild) {
